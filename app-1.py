...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("""
        - Правильный → стрик +1, интервал = 3^(стрик−1) мин  
        - Неправильный → стрик = 0, сразу снова  
        - Следующая: ближайшая просроченная,  
    ...
    if st.session_state.phase == "question":
        ...
        if st.button("➡️ Следующий вопрос", type="primary", use_container_width=True):
            st.session_state.current_card = None  # принудительно выбрать новую
            st.session_state.phase = "question"
            st.session_state.last_feedback = None
            st.rerun()
